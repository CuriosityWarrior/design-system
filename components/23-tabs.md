...
| 원칙 | 설명 |
|---|---|
| 변형 일관성 유지 | 언더라인과 Pills 변형을 한 화면에 혼용하지 않는다. 페이지 수준의 콘텐츠 전환에는 언더라인, 필터나 토글 성격에는 Pills를 사용한다. |
| 탭 수 제한 | 탭은 2~7개 범위에서 사용한다. 탭이 너무 많으면 Dropdown이나 Side Navigation을 대신 고려한다. |
| 탭 레이블 명확성 | 탭 레이블은 콘텐츠를 예측할 수 있도록 명확하고 간결하게 작성한다. 아이콘만 사용하는 경우 툴팁으로 레이블을 제공한다. |
| Navigation Bar와 구분 | Tabs는 동일 페이지 내 콘텐츠 섹션 전환에 사용한다. 앱의 최상위 목적지 간 이동에는 Navigation Bar를 사용한다. |
| 활성 탭 콘텐츠 유지 | 탭 전환 시 비활성 탭의 콘텐츠 상태(스크롤 위치, 입력값 등)를 가능한 유지하여 사용자 컨텍스트를 보존한다. |
