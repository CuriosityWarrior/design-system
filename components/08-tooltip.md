...
| 원칙 | 설명 |
|---|---|
| 보조 정보 한정 | 툴팁은 핵심 정보가 아닌 보조 맥락 제공에만 사용. 반드시 알아야 하는 정보는 툴팁 대신 레이블·힌트 텍스트로 노출한다. |
| 간결한 텍스트 | 툴팁 텍스트는 한두 문장 이내로 유지. 길고 복잡한 내용은 팝오버나 별도 설명 영역을 사용한다. |
| 키보드 접근성 | 포커스 시에도 툴팁이 표시돼야 한다. 마우스 호버 전용으로만 구현하지 않는다. |
| 접근성 마크업 | 툴팁 요소에 `role="tooltip"`, 트리거에 `aria-describedby`로 툴팁 ID를 연결한다. |
| 위치 충돌 방지 | 뷰포트 경계에 가까운 요소에는 Bottom 변형을 사용해 툴팁이 잘리지 않도록 한다. |
| 최대 너비 준수 | 툴팁 최대 너비는 240px로 제한. 임의 Fixed 값으로 너비를 고정하지 않는다. |
