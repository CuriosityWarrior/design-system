...
| 원칙 | 설명 |
|---|---|
| 단일 선택 전용 | Select는 단일 옵션 선택에만 사용. 다중 선택이 필요한 경우 Checkbox 그룹 또는 멀티 셀렉트를 사용한다. |
| 옵션 수 기준 | 선택지가 5개 이하이면 Radio 그룹을 검토. 선택지가 많아 스크롤이 필요한 경우 Select가 적합하다. |
| 스타일 일관성 | 같은 폼 내에서 Input과 동일한 스타일 변형(Border 또는 Surface)을 사용한다. |
| 아이콘 규칙 | 드롭다운 쉐브론은 반드시 `01 — Icons` 페이지의 `expand_more` 인스턴스 사용. 텍스트 기호(▼, ▾) 사용 금지. |
| 크기 선택 | Input과 동일한 크기 체계 사용(L·M·S). 임의 px로 Height를 고정하지 않는다. |
| 접근성 | 레이블을 `for`/`id`로 연결 필수. 비활성 상태에는 `disabled` 속성 적용. |
