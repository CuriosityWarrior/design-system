...
| 원칙 | 설명 |
|---|---|
| 관련 콘텐츠 그룹화 | Card는 하나의 주제나 항목에 대한 관련 정보를 묶는 용도로만 사용한다. 서로 다른 주제를 하나의 Card에 혼합하지 않는다. |
| 클릭 가능 여부 명확히 구분 | 탐색이나 상세 보기로 이동하는 카드는 Clickable 변형을 사용하고, 정보 표시만 하는 카드는 Default 변형을 사용한다. 시각적 커서와 호버 효과가 사용자의 기대를 설정한다. |
| 헤더/푸터는 선택적 사용 | 모든 Card에 헤더와 푸터가 필요한 것은 아니다. 단순 콘텐츠 표시에는 본문만 사용하고 제목이나 액션이 필요한 경우에만 헤더/푸터를 추가한다. |
| 중첩 금지 | Card 안에 Card를 중첩해서 사용하지 않는다. 계층 구조가 필요한 경우 List Item이나 다른 패턴을 활용한다. |
| 높이 고정 지양 | Card의 높이는 내부 콘텐츠에 맞게 자동 조정(HUG)되어야 한다. 그리드에서 높이를 맞춰야 할 경우 부모 그리드에서 처리한다. |
