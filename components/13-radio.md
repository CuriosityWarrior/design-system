...
| 원칙 | 설명 |
|---|---|
| 상호 배타적 선택 | 그룹 내에서 하나만 선택 가능한 경우에 사용. 여러 항목을 독립적으로 선택할 수 있다면 Checkbox를 사용한다. |
| 그룹 레이블 필수 | 라디오 그룹 전체를 설명하는 그룹 레이블(heading-sm)을 항상 위에 배치한다. |
| 항상 하나 선택 | 라디오 그룹은 기본값을 반드시 하나 선택된 상태로 시작. 모두 미선택 상태로 두지 않는다. 초기값이 없어야 할 때는 Select를 검토한다. |
| 크기 일관성 | 같은 그룹 내 라디오 크기는 통일. Size 토큰으로 지정하고 임의 px 고정 금지. |
| 키보드 접근성 | 탭으로 그룹 진입, 화살표 키로 옵션 이동이 가능해야 한다. 포커스 시 `shadow/focus` 링을 표시한다. |
