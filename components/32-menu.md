...
| 원칙 | 설명 |
|---|---|
| 액션 모음에만 사용 | Menu는 수행할 액션의 목록에 사용한다. 값을 선택하는 경우(폼의 옵션 선택 등)에는 Select 컴포넌트를 사용한다. |
| 파괴적 액션 구분 | 삭제, 초기화 등 되돌릴 수 없는 액션은 Destructive 변형으로 분리하고 항목 목록의 하단(Divider 아래)에 배치한다. |
| 서브메뉴 depth 제한 | 서브메뉴는 최대 1단계까지만 사용한다. 2단계 이상 중첩되면 사용성이 급격히 저하된다. |
| 관련 항목 그룹화 | Divider를 활용해 관련 있는 메뉴 아이템을 그룹으로 묶고 논리적인 순서로 배치한다. |
| 키보드 탐색 지원 | 메뉴가 열리면 첫 번째 항목에 자동 포커스되어야 하며, 방향키로 이동하고 Enter로 선택, Esc로 닫을 수 있어야 한다. |
