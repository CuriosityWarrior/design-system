...
| 원칙 | 설명 |
|---|---|
| 스타일 계층 준수 | 한 화면에서 Fill → Border → Text 순으로 시각 강도를 낮춰 사용. 동등한 위계의 버튼에 Fill과 Border를 혼용하지 않는다. |
| 강조 수준 일치 | Primary는 화면당 하나의 주요 CTA에만 사용. Secondary는 보조 액션, Danger는 되돌리기 어려운 위험 액션에만 적용. |
| 크기 선택 | 주 콘텐츠 영역·모달에는 L 이상, 컴팩트 UI(테이블 행·태그 내)에는 M·S 사용. 임의 px로 크기를 고정하지 않는다. |
| Disabled 처리 | 비활성 버튼은 `aria-disabled="true"`를 사용하고 opacity 0.4를 적용. 클릭 이벤트는 차단하되 포커스는 허용한다. |
| Loading 상태 | 비동기 액션 처리 중에는 텍스트를 숨기고 스피너로 대체. 중복 제출을 막기 위해 버튼을 비활성화한다. |
| 아이콘 규칙 | 아이콘은 반드시 `01 — Icons` 페이지의 인스턴스를 사용. 유니코드·이모지·직접 그린 벡터 금지. |
