...
| 원칙 | 설명 |
|---|---|
| 모바일 전용 패턴 | Bottom Sheet는 모바일/터치 환경에 최적화된 패턴이다. 데스크톱에서는 Modal 또는 우측 Drawer를 사용한다. |
| Modal과 Standard 구분 | 사용자의 다른 인터랙션을 차단해야 하는 경우 Modal 변형(스크림 포함)을, 메인 콘텐츠와 함께 공존해야 하는 경우 Standard 변형을 사용한다. |
| 드래그 핸들은 드래그 가능 표시 | Expandable 변형에는 항상 Drag Handle을 표시하여 사용자가 드래그로 높이를 조절할 수 있음을 시각적으로 안내한다. |
| 스냅 포인트 활용 | Expandable 변형의 스냅 포인트(Peek, Half, Full)를 활용하여 콘텐츠의 중요도에 따른 초기 높이를 설정한다. 요약 정보는 Peek, 상세 콘텐츠는 Half 이상으로 시작한다. |
| 키보드 접근성 | 드래그 핸들은 스크린리더에서 닫기 버튼으로 대체 노출하고, Esc 키로 닫기, Tab 순환은 시트 내부로 트랩한다. |
