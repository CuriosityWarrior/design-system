...
| 원칙 | 설명 |
|---|---|
| 단일 행 입력 구분 | 짧은 텍스트(이름·이메일 등)에는 Input을 사용. Text Area는 여러 줄이 필요한 설명·댓글·메시지 입력에만 사용한다. |
| 크기 선택 | 최소 입력 영역 확보를 위해 S(60px), M(80px), L(120px) 중 콘텐츠 특성에 맞게 선택. HUG 대신 Fixed Height를 사용한다. |
| 크기 조정 제한 | 사용자의 세로 방향 크기 조정만 허용(resize: vertical). 가로 조정은 레이아웃을 깨뜨리므로 금지한다. |
| 글자수 카운터 | 최대 입력 글자수가 있는 경우 반드시 카운터를 표시하고, 초과 시 색상을 `color/error/text`로 변경해 즉시 피드백한다. |
| 스타일 일관성 | 같은 폼 내에서 Input과 동일한 스타일 변형(Border 또는 Surface)을 사용한다. |
| 레이블 및 접근성 | Input과 동일하게 `<label>` 연결 필수. 오류 상태에는 `aria-invalid="true"` 및 오류 메시지 연결. |
