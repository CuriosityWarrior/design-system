# 컴포넌트: 아바타 (Avatar)

## 개요
사진 또는 이니셜 폴백으로 사용자를 표현. 프로필, 목록, 댓글 등에 사용.

---

## 디자인 토큰

| 토큰 | 값 |
|---|---|
| 보더 반경 | 50% (원형) |
| 폰트 굵기 | SemiBold 600 |
| 보더 | 2px solid `color/surface/1` (겹친 아바타 구분용) |
| 기본 배경 | `color/primary/tint` |
| 기본 텍스트 | `color/primary/default` |

---

## 레이아웃 규칙

| 항목 | 값 |
|---|---|
| layoutSizingHorizontal | `FIXED` |
| layoutSizingVertical | `FIXED` |

> 정사각형 요소로, Width와 Height를 동일한 Fixed 값으로 설정한다.

## 크기

> 📐 Width와 Height 모두 [파운데이션: 사이즈](../foundations/07-size.md)의 Semantic 토큰을 참조한다 (정사각형).

| 크기 | Size 토큰 | 가로 × 세로 | 폰트 크기 |
|---|---|---|---|
| XL | `size/avatar/XL` → `size/XL` | 48 × 48px | 18px |
| Large (L) | `size/avatar/L` → `size/L` | 40 × 40px | 16px |
| Medium (M) | `size/avatar/M` → `size/M` | 32 × 32px | 13px |
| Small (S) | `size/avatar/S` → `size/S` | 24 × 24px | 11px |
| XS | `size/avatar/XS` → `size/XS` | 16 × 16px | 9px |

> Avatar는 5종 사이즈만 사용 (XL~XS). `02 — Typography` Foundation의 `size/*` Semantic 토큰 7종(XXL/XL/L/M/S/XS/XXS)은 그대로 유지되며, 다른 컴포넌트(예: Icon)가 더 작은 사이즈를 사용한다.

---

## 종류

### 이미지 아바타
- 사용자 사진을 원형으로 크롭, object-fit: cover

### 이니셜 아바타
- 1~2글자 이니셜 중앙 정렬
- 배경: `color/primary/tint` (기본) 또는 사용자별 색상

### 폴백
- 이미지 로드 실패 → 이니셜 표시
- 이니셜 없음 → 기본 사람 아이콘

---

## 온라인 상태 표시
- 위치: 아바타 우측 하단
- 크기: 10px 원형, 흰색 보더 2px
- 온라인: `color/success/default` (초록)
- 자리 비움: `color/warning/default` (앰버)
- 오프라인: `color/text/tertiary` (회색)

---

## 아바타 그룹
- 겹치는 스택: 이후 아바타마다 margin-left -8px
- 초과 표시: `+N` 형태, 중립 회색 스타일 (같은 크기)

---

## 접근성
- 이미지 아바타: `alt` 텍스트 필수
- 이니셜 아바타: `aria-label` 필수 (예: `aria-label="김지수"`)

---

## 아이콘 사용 규칙

> 컴포넌트 내 아이콘은 반드시 `01 — Icons` 페이지의 아이콘 컴포넌트 인스턴스를 사용한다.
> 텍스트 특수 문자(✓, ✕, →, ⋯ 등), 이모지, 직접 그린 벡터 도형으로 아이콘을 대체하는 것을 금지한다.
> 필요한 아이콘이 없는 경우, 먼저 `01 — Icons` 페이지에 추가한 후 인스턴스를 참조한다.

---

### Variants 구성
- 모든 변형은 Figma의 **Combine as Variants** 기능을 사용하여 하나의 Component Set으로 통합한다.

---

## 사용 원칙

| 원칙 | 설명 |
|---|---|
| 폴백 처리 | 이미지 로드 실패 시 이니셜로, 이니셜도 없을 시 기본 아이콘으로 자동 폴백되도록 설계한다. |
| 접근성 필수 | 이미지 아바타는 `alt` 텍스트, 이니셜 아바타는 `aria-label`을 반드시 제공해 사용자 이름을 전달한다. |
| 크기 일관성 | 동일 컨텍스트(목록·댓글·프로필 등)에서 아바타 크기는 통일. Size 토큰으로만 크기를 지정하고 임의 px 고정 금지. |
| 그룹 표시 제한 | 아바타 그룹에서 개수가 넘칠 경우 `+N` 형태로만 표시. 겹침 간격은 -8px로 고정. |
| 온라인 상태 오용 금지 | 온라인 상태 표시는 실시간 상태가 명확한 경우에만 사용. 장식 목적으로 상태 점을 임의 색으로 변경하지 않는다. |
| 아이콘 규칙 | 폴백 아이콘은 `01 — Icons` 페이지의 인스턴스를 사용. 이모지·직접 그린 벡터로 대체 금지. |

---

## Figma Make 프롬프트

```
다음 스펙으로 아바타(Avatar) 컴포넌트를 만들어줘:

크기: XL(48px), Large(40px), Medium(32px), Small(24px), XS(16px)
Width/Height에는 Size/Semantic Variables (size/avatar/*)를 바인딩
형태: 완전한 원형 (border-radius 50%)
폰트: Pretendard SemiBold 600
보더: 흰색 2px 외곽선 (겹침 구분용)

종류:
1. 이미지 — 원형 크롭 사진
2. 이니셜 — 중앙 정렬 1-2글자, 오렌지 틴트 배경(#FFE4C8), 오렌지 텍스트(#F26A00)

온라인 상태 표시 (선택):
- 10px 원형, 우측 하단 위치, 흰색 보더 2px
- 초록(온라인), 앰버(자리 비움), 회색(오프라인)

아바타 그룹 변형:
- 4-5개 아바타를 -8px 간격으로 겹침
- "+N" 초과 표시를 마지막에 중립 스타일로

네이밍: Avatar / {종류} / {크기}
```
