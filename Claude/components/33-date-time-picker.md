...
| 원칙 | 설명 |
|---|---|
| 필요한 단위만 표시 | 날짜만 필요하면 Date Picker, 시간만 필요하면 Time Picker, 둘 다 필요하면 Date & Time Picker를 사용한다. 불필요한 입력 요소를 제공하지 않는다. |
| 범위 선택에 Date Range Picker 사용 | 시작일과 종료일이 모두 필요한 경우 단일 Date Picker 두 개 대신 Date Range Picker를 사용한다. 두 날짜의 선후 관계가 자동으로 강제된다. |
| 최소/최대 날짜 범위 설정 | 선택 가능한 날짜 범위를 비즈니스 로직에 맞게 제한한다. 범위 밖 날짜는 비활성 상태로 표시하여 잘못된 선택을 방지한다. |
| 선택된 값 트리거에 즉시 반영 | 날짜/시간 선택 즉시 트리거 인풋에 선택 값을 표시한다. 팝오버를 닫은 후 적용하는 방식은 지양한다. |
| 직접 입력 허용 검토 | 가능하면 트리거 인풋에서 직접 날짜를 타이핑할 수 있도록 허용하여 키보드 사용자의 편의성을 높인다. |
