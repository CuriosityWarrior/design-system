...
| 원칙 | 설명 |
|---|---|
| 컨텍스트별 종류 선택 | 전체 페이지·섹션 로딩에는 원형 스피너, 인라인·최소 영역에는 점 스피너, 버튼 내부 로딩에는 흰색 변형을 사용한다. |
| 접근성 필수 | 스피너에 `aria-busy="true"`, `aria-label="로딩 중"` 을 반드시 함께 사용해 스크린 리더에 상태를 전달한다. |
| 색상 변형 선택 | 밝은 배경에는 Primary 또는 무채색, 어두운·컬러 배경 위에는 흰색 변형을 사용해 대비를 확보한다. |
| 크기 고정 | 스피너는 FIXED 레이아웃. Size 토큰(`size/spinner/*`)으로만 크기를 지정하며 HUG 사용 금지. |
| 무한 표시 금지 | 로딩이 완료되면 즉시 스피너를 제거. 완료·실패 상태를 사용자에게 명확히 전달한다. |
