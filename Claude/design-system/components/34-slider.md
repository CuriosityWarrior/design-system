...
| 원칙 | 설명 |
|---|---|
| 연속적인 범위 값에 사용 | Slider는 볼륨, 밝기, 가격 범위처럼 연속적인 값 선택에 적합하다. 정확한 숫자 입력이 필요하면 Number Input을 사용한다. |
| 현재 값 항상 표시 | 사용자가 선택한 값을 즉시 알 수 있도록 Value Label을 표시하거나 인접한 텍스트로 현재 값을 표시한다. |
| 범위 제한 명확히 안내 | 최솟값과 최댓값을 Slider 양 끝에 표시하여 사용자가 선택 가능한 범위를 직관적으로 파악할 수 있게 한다. |
| 키보드 접근성 확보 | 마우스 드래그 외에도 방향키로 값을 조정할 수 있도록 구현하고, aria 속성을 반드시 포함한다. |
| 스텝이 중요한 경우 눈금 표시 | 선택 가능한 값이 특정 간격(예: 10 단위)으로 제한된 경우 With Ticks 변형을 사용하여 선택 가능한 포인트를 시각화한다. |
