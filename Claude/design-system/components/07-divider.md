...
| 원칙 | 설명 |
|---|---|
| 강도 선택 | 주요 섹션 경계에는 Default 또는 Strong, 목록 항목 구분 등 저강조 영역에는 Subtle을 사용한다. |
| 레이블 구분선 | "또는(OR)" 등 맥락 분리 목적에만 레이블 포함 변형을 사용. 레이블 없이 장식 목적으로 사용 금지. |
| 방향 일치 | 가로 콘텐츠 흐름 구분에는 Horizontal, 인라인 요소 구분에는 Vertical을 사용. 방향을 혼용하지 않는다. |
| 접근성 | 의미 있는 구분선은 `<hr>` 또는 `role="separator"` 사용. 장식용 구분선은 `aria-hidden="true"` 처리. |
| 크기 토큰 준수 | 두께는 `size/border/thin` 토큰만 사용. 임의 px 값으로 두께를 변경하지 않는다. |
| 과도한 사용 금지 | 구분선 남용은 시각적 잡음을 유발한다. 여백(spacing)만으로 충분히 구분 가능한 경우 구분선을 추가하지 않는다. |
